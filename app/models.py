...
class Movie(db.Model):
    ...
    addtime = db.Column(db.DateTime, index=True, default=datetime.uncnow)  # 添加时间
...
class Moviecol(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # 编号
    content = db.Column(db.Text)  # 内容
    movie_id = db.Column(db.Integer, db.ForeignKey('movie_id'))  # 所属电影
    user_id = db.Column(db.Integer, db.ForeignKey('user_id'))  # 所属用户
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 添加时间
...
class Auth(db.Model):
    ...
    url = db.Column(db.String(255), unique=True)  # 地址
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 添加事假
